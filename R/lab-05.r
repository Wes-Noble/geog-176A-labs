...
imagery = getlandsat::lsat_scenes()
...
scene = imagery %>%
  filter(min_lon <= bb$xmin, max_lon >= bb$xmax, min_lat <= bb$ymin, max_lat >= bb$ymax,
         as.Date(acquisitionDate) == as.Date("2016-09-26"))


write.csv(scene, file = "/users/noblex/github/geog-176A-labs/data/palo-flood-scene.csv", row.names = FALSE)
...
plot(kmeans_raster1, col = palette(150))
